...
def solve(dim, dat):
    ...
    PART 1
    traverse tree in post order and record each node's depth and its deepest child,
    this finds also the longest path from root to leaf
    
    PART 2
    find largest sum of 3 branches assuming they all go into the longest path DIRECTLY
    
    PART 3
    find the largest sum of 3 branches assuming that there is a secondary branching on the
    second longest branch 
    
    PART 4
    compare results of PART 2 and 3 and return the larger
...
##### Find 3 longest branches from longest path #####
    ...
    for i in reversed(path):
        path1.append(i[0])
    path1_total, res1, path_r1 = find_length(path1)
    
    path2 = find_path(res1) # find second longest path
    path2_total, res2, path_r2 = find_length(path2) # length of 2 branches on the second longest branch
    path2_total = res2[0]+path_r1  # path2_total = (second longest branch with branch) + (longest branch)

## PART 4 ##
##### Compare results of PARTS 2 and 3 #####
    if res2 == 0: # if (second longest branch with branch) = 0 then only 3 branch path is possible
        res = path1_total
    else:
        res = max(path2_total, path1_total)
...
def find_length(path):
    ...
    return path3_total, resq, path_r
