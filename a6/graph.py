...
class Graph:
    ...
    def addedge(self, inode1, inode2, dist):
        node1 = self.nodes[inode1]
        node2 = self.nodes[inode2]
        node1.neighbours.append((node2,dist))
        node2.neighbours.append((node1,dist))
...
def BFS3(graph, nodeId, bool_servers, meta, key_servers):
    v = 999
    dists = {k:v for k in range(meta[0])}
    key_dists = {}
    key_dists[nodeId] = 0
    dists[nodeId] = 0
    queue = Queue(200)
    ...
    while(not queue.isEmpty()):
        ...
        tmp_path = 0     
        for neigh in node.neighbours: 
            tmp_dist = dists[node.id] + neigh[1]
            if dists[neigh[0].id] > tmp_dist:
                dists[neigh[0].id] = tmp_dist
            if bool_servers[neigh[0].id] == False:
                queue.Enqueue(neigh[0])
            else:
                try:
                    key_dists[neigh[0].id]
                    if key_dists[neigh[0].id] > tmp_dist:
                        key_dists[neigh[0].id] = tmp_dist
                except:
                    key_dists[neigh[0].id] = tmp_dist
    print(key_dists)
    return key_dists

def BFS2(graph, nodeId, ends):
    ends = ends.copy()
    dists2ends = {}
    dists = {}
    dists[nodeId] = 1
    ...
    while(not queue.isEmpty()):
        ...
        for neigh in node.neighbours:
            if not visited[neigh.id]:
                queue.Enqueue(neigh)
                visited[neigh.id] = True
                dists[neigh.id] = dists[node.id] + 1
                
                if neigh.id in ends:
                    ends.remove(neigh.id)
                    dists2ends[neigh.id] = dists[neigh.id]
                    if ends == []:
                        return dists2ends
